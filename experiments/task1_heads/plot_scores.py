...
One row per head, on the walnut ensemble config with both volume normalizations, where the
sweep reaches a perfect ranking. Covariates come from `experiments/explore_fomo_task1/explore.tsv`;
only the probabilities differ between runs, so a column reads straight down the sweep.
"""
...
ORDER = (
    "walnut-ensemble_head-cv-auc",
    "walnut-ensemble_head-fixed-C0.01",
    "walnut-ensemble_head-fixed-C1",
    "walnut-ensemble_head-fixed-C100",
)
...
def main() -> None:
    ...
    for row, name in enumerate(ORDER):
        ...
        axes[row, 0].set_ylabel(
            f"{name.removeprefix('walnut-ensemble_head-')}\n\nout-of-fold p", fontsize=8
        )
    ...
    fig.suptitle(
        "Task 1 heads: walnut-vitl, ensemble pooling, both volume normalizations, scaler on",
        y=1.002,
    )
    fig.tight_layout()
